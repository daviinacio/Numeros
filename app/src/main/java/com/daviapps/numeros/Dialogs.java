...
class ScoreDialog extends Dialog {
	...
	/*private void updateLista(){
		int list_height = Utils.dpToPixel(getContext(), 41), list_length = 7;
		if(adapter.getCount() >= list_length){}
		else if(adapter.getCount() == 0)
			list_length = 1;
		else
			list_length = adapter.getCount();
		list_height *= list_length;
		list.setAdapter(adapter);
		findViewById(R.id.pontos_lista_nenhumPonto).setVisibility(adapter.getCount() > 0 ? View.GONE : View.VISIBLE);
		((RelativeLayout) findViewById(R.id.pontos_lista_lay)).setLayoutParams(new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, list_height));
	}*/
}
	...
	public PerdeuDialog(final Context context, Score pt){
		...
		players.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener(){
			...
			public void onItemSelected(AdapterView <?> p1, View v, int i, long p4){
				...
				MainActivity.json_prefs.put(MainActivity.PREFS_PLAYER, pAdapter.getItem(i).getId());
				//MainActivity.preferences.save();
			}
...
class PlayerAdapter extends BaseAdapter implements AdapterBaseStruct<OldPlayer> {
	...
	public View getView(final int i, View arg1, ViewGroup arg2) {
		...
		/*name.setOnClickListener(new View.OnClickListener(){
			...
			public void onClick(View v){
				if(i == 0)
					Toast.makeText(context, "Novo player", Toast.LENGTH_SHORT).show();
				onItemClick(i);
			}
		});*/
...
abstract class DDBTableBase <T> {
	...
	/*protected DDBTableBase(String filename){
		this(new File(Environment.getExternalStorageDirectory(), "android/data/com.daviapps.numeros/" + filename));
	}*/
